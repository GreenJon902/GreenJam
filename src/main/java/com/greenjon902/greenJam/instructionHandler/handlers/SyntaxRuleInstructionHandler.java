...
public class SyntaxRuleInstructionHandler extends InstructionHandlerBase {
    private final SyntaxContext syntaxContext;

    public SyntaxRuleInstructionHandler(SyntaxContext syntaxContext) {
    ...
    public void handle(InstructionTokenInputStream tokens) {
        ...
        if (!tokens.hasLeft(2)) {
            Errors.instructionHandler_invalidInstructionAmountLeftForType(tokens, "syntax", 2);
        }
        if (tokens.next().type != InstructionToken.InstructionTokenType.IDENTIFIER) {
            Errors.instructionHandler_invalidTokenAtLocation(tokens);
        }
        if (tokens.next(1).type != InstructionToken.InstructionTokenType.SYNTAX_RULE) {
            Errors.instructionHandler_invalidTokenAtLocation(tokens);
        }

        String group = (String) tokens.consume().storage;
        SyntaxRule rule = (SyntaxRule) tokens.consume().storage;

        switch ((InstructionKeyword) instruction.storage) {
            case ADD -> syntaxContext.add(group, rule);
            case REMOVE -> syntaxContext.remove(group, rule);
            default -> {
                tokens.location -= 2;
                Errors.instructionHandler_invalidTokenAtLocation(tokens);
            }
